  ...
  return (
    <div className="component">
      ...
      <div >
        ...
        {income && <p>Gross Income: {formatINR(numericIncome + rebate)}</p>}
        {totalTax > 0 && <p>Total Tax: {formatINR(totalTax)}</p>}
        {cess > 0 && <p>Cess (4%): {formatINR(cess)}</p>}
        {taxWithCess > 0 && <p>Total Tax with Cess: {formatINR(taxWithCess)}</p>}
        {taxWithCess > 0 && <p>After-tax Income: {formatINR(afterTaxIncome)}</p>}
        {propertyValue && income && `Rental Yield (4% - 6%) : ${((afterTaxIncome) / (propertyValue.replaceAll(",", "")) * 100).toFixed(2)}%`}
        {taxWithCess > 0 && <p>After-tax Monthly Income: {formatINR((afterTaxIncome / 12).toFixed(0))}</p>}
      </div>
    </div>
  );
}
